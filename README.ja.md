...
**プレミアム機能を除き、Open Reading はすべてオープンソースです（全体の約 99%）。**
リーダーを構成するほとんどの部分は本リポジトリにあり、[AGPL-3.0](LICENSE) で公開されて
います。公開していないのは 2 つだけです。**プレミアム機能**（有料会員向け機能とその実装）
と**バックエンド**（アカウント、会員確認、購入検証を担うサーバー）です。どちらも読書
そのものには影響しません。本リポジトリのリーダーはログイン不要で、当方のクラウドに依存
せず、オフラインでも完全に動作します。

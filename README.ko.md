...
**프리미엄 기능을 제외한 모든 것이 오픈 소스이며, 전체의 약 99%입니다.** 이 리더를
구성하는 대부분이 본 저장소에 [AGPL-3.0](LICENSE)으로 공개되어 있습니다. 공개하지 않은
것은 두 가지뿐입니다. **프리미엄 기능**(유료 멤버십 기능과 그 구현)과 **백엔드
서비스**(계정, 멤버십 확인, 구매 검증을 담당하는 서버)입니다. 둘 다 읽기 자체에는
영향을 주지 않습니다. 본 저장소의 리더는 로그인이 필요 없고, 저희 클라우드에 의존하지
않으며, 오프라인에서도 완전하게 동작합니다.

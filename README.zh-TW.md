...
**除進階功能外，開元閱讀全部開源，整體開源程度約 99%。** 構成這款閱讀器的絕大部分內容
都在本倉庫，以 [AGPL-3.0](LICENSE) 授權。未開源的只有兩塊：**進階功能**（會員加值能力
及其實作）與**後端服務**（帳號、會員校驗與購買驗證的伺服器）。兩者都不影響閱讀本身 ——
本倉庫裡的閱讀器不需登入、不依賴專案方雲端服務，離線也能完整使用。
